var boom = require('boom'),
  ...
  types = require('./types'),
  debug = require('debug')('mongoscope-brain:replicaset');
...
module.exports.discover = function(db, fn) {
  ...
  add(_.keys(state.secondaries).map(function(hp) {
    return types.url(hp).rs(rs).state('secondary');
  }));

  add(_.keys(state.arbiters).map(function(hp) {
    return types.url(hp).rs(rs).type('arbiter');
  }));
  ...
  add(types.url(state.primary.name).rs(rs).state('primary'));
  debug('Discovered replicaset', {
    instances: instances,
    type: 'replicaset',
    replicaset: rs
  });
